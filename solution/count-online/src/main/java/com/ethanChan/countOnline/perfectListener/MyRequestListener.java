 ...
 * @Description 在Map中，需要使用键值对的方式,Key为IP，Value为List，那么List中存放什么呢？
 *                  存放的是此IP发出的所有回话的HttpSession的对象，所以List的泛型是HttpSession。
 *               请求，在请求中，因为将当前Session 对象存放到List中， List在Map中，Map在全局域中，
 *                  所以首先得从全局域获取到Map，
 *                  然后，从Map中获取由当前IP所发出的所有Session的组成的List，判断当前的List是否为NULL
 *                      ，若为NULL，则创建List
 *                      ，否则，将当前SessioncurrentSession放入List中。
 * @Author EthanChan
 * @Version 1.0.0
